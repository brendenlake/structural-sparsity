...
node_label = [];   arc_label = []; weighted=[];  % set default args
width = 10;        height = 10;
...
for i = 1:2:nargin-1              % get optional args
    switch varargin{i}
        ...
        case 'weighted', weighted = varargin{i+1};
    end
end

assert(isempty(weighted)||isempty(arc_label));
assert(~(~isempty(weighted)&&directed));
...
if directed
    ...
    fprintf(fid, 'graph G {\n');
    arctxt = '--'; 
    if isempty(arc_label) && isempty(weighted)
        labeltxt = '[dir=none]';
    elseif isempty(arc_label)
        labeltxt = '[len="%d",dir=none]';
    else
        labeltext = '[label="%d",dir=none]';
    end
end
...
for node1 = 1:Nnds              % process ARCs
    ...
    for node2 = arcs
        if isempty(arc_label) && isempty(weighted)     % thanks to Nicholas Wayne Henderson nwh@owlnet.rice.edu
            fprintf(fid, edgeformat, node1, node2);  
        elseif isempty(arc_label)
            %Since neato doesn't handle negative weights
            fprintf(fid, edgeformat, node1, node2, abs(weighted(node1,node2)));
        else
